...
def get_search():
    try:
        ...
        query = f'''
            ...
            '''
        query2 = f'''
            MATCH (s:Stock{{code:'{q}'}})-[:subindustry_of]->(si:Subindustry)
            RETURN s, si
            '''
        results = graph.run(query).data()
        results2 = graph.run(query2).data()

        return Response(dumps({"stock_code": results2[0]['s']['code'],
                               "stock_name": results2[0]['s']['name'],
                               "industry": '<br>'.join([result['si']['name'] for result in results2]),
                               "concept": '<br> '.join([result['c']['name'] for result in results])}),
