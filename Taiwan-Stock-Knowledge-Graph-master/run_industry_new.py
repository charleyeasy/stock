...
def serialize_ind(ind,sub,flag):
    if flag==1:
        return {
            'name': ind['name'],#最大
            'subname': sub['name'],#最小
            'submd5': sub['subindustry_new_id'],#序號
        }
    else:
        return{
            'name': 'nan',#最大
...
@app.route('/')
def index():
    return app.send_static_file('index_industry_new.html')
...
def get_search():
    try:
        ...
        return Response(dumps({"stock_code": code,
                               ...
                               "Industry_stream": [serialize_ind(result['i'],result['c'],flag) for result in results]}),
...
if __name__ == '__main__':
    ...
    app.run(host='10.220.26.89',port=5000)
